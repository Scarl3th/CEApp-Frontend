  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      if (modoEdicion) {
        ...
        Alert.alert("Éxito", "Evento editado correctamente.", [{ text: "OK", onPress: () => {
                                                                          if(isProfesional){
                                                                          router.push(`/profesional/${paciente}/calendario?recargar=1`);}
                                                                          else{
                                                                          router.push(`/cuidador/${paciente}/calendario?recargar=1`);}} 
                                                              }]);
      } else {
        ...
        Alert.alert("Éxito", "Evento creado correctamente.", [{ text: "OK", onPress: () => {
                                                                          if(isProfesional){
                                                                          router.push(`/profesional/${paciente}/calendario?recargar=1`);}
                                                                          else{
                                                                          router.push(`/cuidador/${paciente}/calendario?recargar=1`);}} 
      ...
      Alert.alert("Error", modoEdicion ? "No se pudo editar el evento." : "No se pudo crear el evento.", [{ text: "OK" }]);
    } finally {
  ...
  return (
    <DescartarCambiosContext.Provider value={{ handleDescartarCambios }}>
      <KeyboardAwareScrollView className="flex-1" contentContainerStyle={{ flexGrow: 1, padding: 8 }} keyboardShouldPersistTaps="handled" extraScrollHeight={24}>
        <Titulo>{modoEdicion ? "Editar evento" : "Agregar evento"}</Titulo>
        {isLoading ? (
          <IndicadorCarga/>
        ) : (
          <View className="gap-2">
            ...
            {/* Día de la semana (solo si tipo semanal) */}
            {tipo === "semanal" && (
            <FormularioCampoSelect
            ...
            {/* Día del mes (solo si tipo mensual) */}
            {tipo === "mensual" && (
            <FormularioCampo
                label="Día del mes"
